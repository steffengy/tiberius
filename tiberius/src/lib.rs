...
mod tests {
    ...
    fn unexpected_token_test() {
        ...
        let future = SqlConnection::connect(lp.handle(), connection_string.as_str())
            .and_then(|conn| conn.simple_exec("CREATE TABLE #Temp(test int); INSERT INTO #Temp (test) VALUES (42);").and_then(|r| r).collect())
            .and_then(|(_, conn)| conn.query( "DELETE FROM #Temp", &[] )
                .for_each_row( |row| { let _ : &str = row.get(0); Ok(()) } )
            );
        lp.run(future).unwrap();
    }
    #[test]
    fn unreachable_code_test() {
        let mut lp = Core::new().unwrap();
        let connection_string = connection_string();

        let profile = "jake@jake.jake".to_string();
        let logged_id = 1;
...
UNION ALL SELECT 5 as FollowingId, 1 as FollowerId
UNION ALL SELECT 6 as FollowingId, 1 as FollowerId
UNION ALL SELECT 7 as FollowingId, 1 as FollowerId
UNION ALL SELECT 8 as FollowingId, 1 as FollowerId
UNION ALL SELECT 9 as FollowingId, 1 as FollowerId
...
UNION ALL
SELECT 3 Id, 'jake@jake.jake' [Email],'jake@jake.jake' [Token],'jake@jake.jake' [UserName], NULL [Bio],NULL [Image]
UNION ALL
SELECT 4 Id, 'jake@jake.jake' [Email],'jake@jake.jake' [Token],'jake@jake.jake' [UserName], NULL [Bio],NULL [Image]
UNION ALL
SELECT 5 Id, 'jake@jake.jake' [Email],'jake@jake.jake' [Token],'jake@jake.jake' [UserName], NULL [Bio],NULL [Image]
UNION ALL
SELECT 6 Id, 'jake@jake.jake' [Email],'jake@jake.jake' [Token],'jake@jake.jake' [UserName], NULL [Bio],NULL [Image]
UNION ALL
SELECT 7 Id, 'jake@jake.jake' [Email],'jake@jake.jake' [Token],'jake@jake.jake' [UserName], NULL [Bio],NULL [Image]
UNION ALL
SELECT 8 Id, 'jake@jake.jake' [Email],'jake@jake.jake' [Token],'jake@jake.jake' [UserName], NULL [Bio],NULL [Image]
UNION ALL
SELECT 9 Id, 'jake@jake.jake' [Email],'jake@jake.jake' [Token],'jake@jake.jake' [UserName], NULL [Bio],NULL [Image]
UNION ALL
SELECT 10 Id, 'jake@jake.jake' [Email],'jake@jake.jake' [Token],'jake@jake.jake' [UserName], NULL [Bio],NULL [Image]
UNION ALL
